...
class AIVoiceAssistant:
    ...
    def _create_kb(self):
        try:
            reader = SimpleDirectoryReader(
                input_files=[r"D:\ENIT.txt"]
            )
            documents = reader.load_data()
            vector_store = QdrantVectorStore(client=self._client, collection_name="Hospitality")
    ...
    def _prompt(self):
        return """
            You are a professional AI Hospitality Robot Named ENIT ! always ask question one by one only!
